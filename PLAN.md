...
방장:   [홈] → "각자 폰으로" → [닉네임+동물] → [대기실(공유)] → [게임선택] → [게임진행] → [결과]
친구:   [공유 링크 클릭] → [닉네임+동물] → [대기실] → (방장이 게임 시작) → [게임진행] → [결과]
```
- 방장이 "각자 폰으로" 선택 시 자동으로 방 생성 + 코드/링크/QR 발급
- 친구들은 **공유 링크 클릭 한 번**으로 바로 입장 (방 검색/조회 없음)
- 각자 폰에서 닉네임 입력 + 동물 선택 → 대기실 자동 합류
- 방장이 게임 선택 → 모두 동시 진행
- 실시간 점수/상태 동기화
...
### 6.1 핵심 원칙
- **게임 생성자(방장)만** 모드 선택 (싱글/멀티)
- **참여자는 공유 링크 클릭 → 닉네임 + 동물 선택 → 바로 입장** (방 조회/검색 없음)
- 방은 초대 기반 비공개, 코드를 아는 사람만 입장 가능
...
[홈] ─┬─ "한 폰으로" → [인원설정 + 캐릭터선택] → [게임선택] → [게임진행] → [결과]
      │                                                                        ↓
      └─ "각자 폰으로" → [닉네임 + 동물선택] → [대기실(공유)] → [게임선택] → [게임진행] → [결과]
                                                    ↑
=== 참여자 (친구) ===                               │
                                                    │
[공유 링크 클릭] → [닉네임 + 동물선택] → ───────────┘
...
- **"각자 폰으로!"** 버튼 (멀티 모드)
- **"게임 방법"** 링크
- 이 화면은 **게임을 만드는 사람만** 보는 화면

#### 싱글 - 인원 설정 + 캐릭터 선택 (/solo/setup)
- 2~8명 숫자 선택 (큰 원형 버튼으로 +/-)
- 각 인원 이름 입력 (선택사항, 기본값: "Player 1", "Player 2"...)
- 동물 캐릭터 선택: 순서대로 선택 or **"전체 랜덤 배정"** 버튼
- 선택된 동물은 비활성화 (회색 처리)
- **"게임 선택으로!"** 버튼

#### 멀티 - 대기실 (/room/[code])
- **방장 진입 시**: 닉네임 + 동물 선택 → 방 자동 생성 → 대기실 표시
- **참여자 진입 시**: 공유 링크로 접근 → 닉네임 + 동물 선택 → 바로 대기실 입장
- **공유 영역** (방장에게만 강조 표시):
  - 방 코드 (예: `CAFE42`) + 복사 버튼
  - 링크 복사 버튼
  - QR 코드
...
- 방장 표시 (왕관 아이콘)
- 방장만 **"게임 시작!"** 버튼 표시 (2명 이상 시 활성화)
- **코드 직접 입력**: 링크가 안 될 때를 위한 보조 수단 (하단 작은 링크)
...
│   ├── page.tsx                    # 홈 화면 (모드 선택 — 생성자만)
│   ├── solo/
│   │   └── setup/
│   │       └── page.tsx            # 인원 설정 + 캐릭터 선택
│   ├── room/
│   │   └── [code]/
│   │       └── page.tsx            # 대기실 (방장: 생성+대기, 참여자: 링크→입장)
│   ├── games/
│   │   ├── page.tsx                # 게임 선택 화면
│   │   ├── tap-race/
│   │   │   └── page.tsx            # 달리기 경주
...
- [ ] 대기실 화면 (방장: 방 생성 + 공유 패널 / 참여자: 링크로 바로 입장)
- [ ] 공유 링크 진입 → 닉네임 + 동물 선택 → 대기실 자동 합류
